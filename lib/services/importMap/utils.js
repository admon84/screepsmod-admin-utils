...
const logStart = async (config, mapId) => {
  if (!config.mongo) {
    throw new Error('Error: screepsmod-mongo required for map imports')
  }
  log(`Importing map: ${mapId}`)
}

const logFinish = () => {
  return logResult('Import complete: restart the server and use system.resumeSimulation() to unpause ticks')
}
...
const getMapFromUrl = async (urlOrMapId) => {
  ...
  if (urlOrMapId.startsWith('random')) {
    ...
    if (!maps.length) {
      return logResult(`Random map with size ${size} requested, but no maps match requirements`)
    }
...
module.exports = (config) => {
  ...
  config.utils.importMapFile._help =
    'importMapFile(filePath) - import a map from a file'
}
