...
const log = (...v) => console.log('[ImportMap]', ...v)

const insertRooms = async (config, map, rooms) => {
  const {
    common: {
      dbCollections,
  ...
  const accessibleRoomList = rooms
    .filter(
      r => r.status === 'normal' && (!r.openTime || r.openTime < Date.now())
    )
    .map(r => r.room)
...
const getMapFromUrl = async map => {
  let url = map
  if (map.startsWith('random')) {
    const [, size] = map.split('_')
    const [width, height] = size.split('x').map(v => +v || 1)
    const { data } = await axios.get(
      'https://maps.screepspl.us/maps/index.json'
    )

    const maps = Object.values(data).filter(
      m => +m.width === width && +m.height === height
    )
    if (!maps.length) {
      log(
        `Random map with size ${size} requested, but no maps match requirements.`
      )
...
const getMapFromFile = async map => {
  const { rooms } = JSON.parse(await fs.readFile(map, { encoding: 'utf8' }))
  return rooms
}

const startImport = async (config, map) => {
  if (!config.mongo) {
...
module.exports = config => {
  Object.assign(config.utils, {
    async importMap(map) {
      startImport(config, map)
      const rooms = getMapFromUrl(map)
      await insertRooms(config, map, rooms)
      return `Map imported from ${map} with ${rooms.length} rooms`
    },
    async importMapFile(map) {
      startImport(config, map)
      const rooms = getMapFromFile(map)
      await insertRooms(config, map, rooms)
      return `Map imported from ${map} with ${rooms.length} rooms`
    }
  })
  ...
  config.utils.importMapFile._help =
    'importMapFile(mapFile.json) - import a map from a file'
}
